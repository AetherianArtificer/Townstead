...
class ThermalComfortTest {
    ...
    @Test void movementHelpsButDoesNotEraseColdWaterExposure() {
        ...
        assertTrue(load(12, 1, true, 2, ThermalProtection.NONE) <= -12, "Cold water is left at once");
    }
    @Test void wetClothingLosesProtectionAndStaysWetOnLand() {
        ...
        var dry = ThermalComfort.update(new ThermalComfort.State(1, -10, 0), -10, false, false, 20, 1, 90);
    ...
    @Test void immersionActsImmediatelyAndRainSoaksGradually() {
        ...
        var water = ThermalComfort.update(initial, -20, true, false, 12, 0.05f, 90);
        assertEquals(1, water.wetness());
        assertEquals(-20, water.load());
        var rain = ThermalComfort.update(initial, -5, false, true, 15, 1, 90);
    ...
    @Test void dryingIsGradualAndSlowerInColdAir() {
        ...
        var warm = ThermalComfort.update(wet, 0, false, false, 20, 45, 90);
        var cold = ThermalComfort.update(wet, 0, false, false, 5, 45, 90);
        assertEquals(0.5f, warm.wetness(), 0.001f);
        assertTrue(cold.wetness() > warm.wetness());
        assertEquals(0, ThermalComfort.update(warm, 0, false, false, 20, 45, 90).wetness());
    }
