...
/**
 * The family's palette column: a scrolling list of item rows under collapsible category headers.
 *
 * <p>Born in the Field Post and moved here the second a second screen wanted one. The headers, the
 * collapse, the scrollbar and the hover states are the Field Post's, unchanged — which is the point.
 * Two screens a player opens in the same session should not have two kinds of list in them.</p>
 */
public class PaletteList extends ObjectSelectionList<PaletteList.ToolEntry> {
    ...
    //? if >=1.21 {
    public PaletteList(Minecraft mc, int x, int width, int height, int y, Consumer<ToolEntry> onSelectionChanged) {
    ...
    //?} else {
    /*public PaletteList(Minecraft mc, int x, int width, int height, int top, Consumer<ToolEntry> onSelectionChanged) {
    ...
    *///?}
    ...
    public static class ToolEntry extends ObjectSelectionList.Entry<ToolEntry> {
        ...
        public PaletteList parentList;

        /** Greyed out because the world cannot supply it right now. Purely cosmetic. */
        public boolean dim;
        /** Shown while the cursor rests on this row, or null. The screen draws it, not the list. */
        ...
        private void renderTool(GuiGraphics g, Minecraft mc, int top, int left, int width, int height, boolean hovered) {
            ...
            if (hovered && tooltip != null && parentList != null) {
                parentList.hoveredTooltip = tooltip;
            }

            // Label + assignment count
            int textColor = dim ? (hovered ? 0xFF9A9A9A : 0xFF7A7A7A)
    ...
    /**
     * The tooltip of whichever row the cursor is over, cleared as it is read.
     *
     * <p>Collected during the row's own render and handed back rather than drawn in place, because
     * a tooltip drawn inside a scrolling list is clipped by the list it came from.</p>
     */
    @Nullable
    public String takeTooltip() {
        String out = hoveredTooltip;
        hoveredTooltip = null;
        return out;
    }
