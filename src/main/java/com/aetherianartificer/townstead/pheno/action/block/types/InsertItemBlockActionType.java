...
public final class InsertItemBlockActionType implements BlockActionType {
    ...
    @Override public BlockAction parse(JsonObject json) {
        ...
        if (slot < 0 || count < 1 || limit < 1 || role.isBlank() || side == null) return null;
        return new BlockAction() {
            ...
            @Override public boolean canRun(BlockActionContext ctx) {
                ...
                if (stored.getCount() >= limit) return false;
                ...
                offer.setCount(offerCount(supplied.getCount(), stored.getCount(), count, limit));
            ...
            @Override public void run(BlockActionContext ctx) {
                ...
                ItemStack supplied = ctx.itemRole(role);
                int amount = offerCount(supplied.getCount(), inventory.getStackInSlot(slot).getCount(), count, limit);
