...
public final class ButcherToolDamage {
    ...
    public static boolean hasCleaver(VillagerEntityMCA villager) {
        return findFirst(villager, true) >= 0;
    }

    public static boolean hasKnife(VillagerEntityMCA villager) {
        return findFirst(villager, false) >= 0;
    }
    ...
    public static void consumeKnifeUse(VillagerEntityMCA villager) {
        damageFirst(villager, false);
    }

    private static int findFirst(VillagerEntityMCA villager, boolean cleaver) {
        SimpleContainer inv = villager.getInventory();
        for (int i = 0; i < inv.getContainerSize(); i++) {
            ItemStack stack = inv.getItem(i);
            if (stack.isEmpty()) continue;
            if (cleaver ? WorkToolTicker.isCleaver(stack) : WorkToolTicker.isKnife(stack)) {
                return i;
            }
        }
        return -1;
    ...
    private static void damageFirst(VillagerEntityMCA villager, boolean cleaver) {
        int slot = findFirst(villager, cleaver);
        if (slot < 0) return;
        SimpleContainer inv = villager.getInventory();
