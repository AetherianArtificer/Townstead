...
public final class WorkPathing {
    ...
    public static boolean isSafeStandPosition(BlockGetter level, BlockPos feetPos) {
        ...
        if (Stations.avoidStandingSurface(floorState)) return false;
        if (WorkHazards.unsafeSurface(level, floorPos)
                || WorkHazards.unsafeSurface(level, feetPos)
                || WorkHazards.unsafeSurface(level, headPos)) {
            return false;
        }
        ...
        return !WorkHazards.hazardous(level, feetPos)
                && !WorkHazards.hazardous(level, floorPos);
    }
