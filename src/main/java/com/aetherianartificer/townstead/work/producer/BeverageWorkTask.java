...
public class BeverageWorkTask extends ProducerWorkTask {
    ...
    @Override
    protected String debugLabel() { return "Barista"; }
    ...
    protected boolean isEligibleVillager(ServerLevel level, VillagerEntityMCA villager) {
        if (!com.aetherianartificer.townstead.work.WorkTaskDeclarations.permitsTask(
                villager, ProducerTaskDeclarations.BREW)) return false;
        return BaristaAssignment.canVillagerWorkAsBarista(level, villager);
    }
    ...
    protected @Nullable ProducerStationSelection selectStation(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        ...
        for (List<com.aetherianartificer.townstead.profession.def.WorkTaskDef> bucket
                : ProducerTaskDeclarations.brewBuckets(villager)) {
            best = ProducerStationIndex.chooseBaristaSelection(
                    level, villager, cafeSnapshot, cafeBounds, abandonedUntilByStation,
                    gameTime, recipeCooldownUntil, ProducerTaskDeclarations.stationFilter(bucket));
            if (best != null) break;
        }
    ...
    protected @Nullable ProducerRecipe pickRecipe(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        ...
        if (recipe == null) {
            int available = WorkRecipeRegistry.getBeverageRecipesForStation(level, stationType).size();
    ...
    protected GatherResult gatherInputs(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        ...
        if (!pullResult.success()) {
            ...
            String missing = WorkIngredients.describeMissingRequirements(level, villager, recipe, stationAnchor, cafeBounds);
    ...
    protected boolean isProduceDone(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        ...
        if (stationType == StationType.HOT_STATION
                ...
                && !StationHandler.stationHasCollectibleOutput(level, stationAnchor, WorkRecipeRegistry.allOutputIds(level))) {
            return false;
        }
    ...
    @Override
    protected String historyCounter() { return "townstead:brewed"; }
    ...
    protected void onProduceTick(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        if (stationType == StationType.FIRE_STATION && stationAnchor != null) {
            ...
            for (ItemStack drop : drops) {
                WorkIngredients.storeOutputInWorksiteStorage(level, villager, drop, stationAnchor, cafeBounds);
                if (!drop.isEmpty()) {
    ...
    protected void onOpportunisticSweep(ServerLevel level, VillagerEntityMCA villager, long gameTime) {
        ProducerOutputHelper.sweepNearbyOutputs(
                ...
                WorkRecipeRegistry.allOutputIds(level));
    }
    ...
    private Set<Long> activeCafeStorageBounds(VillagerEntityMCA villager) {
        ...
        return BaristaAssignment.assignedCafeBounds(level, villager);
    }
    ...
    private Optional<Building> activeCafeBuilding(ServerLevel level, VillagerEntityMCA villager) {
        return BaristaAssignment.assignedCafe(level, villager);
    }
