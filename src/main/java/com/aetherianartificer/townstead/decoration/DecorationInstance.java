...
public record DecorationInstance(ResourceLocation decorationId, BlockPos anchor, List<BlockPos> members) {
    ...
    public CompoundTag save() {
        ...
        tag.putString("decoration", decorationId.toString());
    ...
    public static @Nullable DecorationInstance load(CompoundTag tag) {
        ResourceLocation decorationId = ResourceLocation.tryParse(tag.getString("decoration"));
        if (decorationId == null || !tag.contains("anchor")) return null;
        List<BlockPos> members = new ArrayList<>();
        for (long packed : tag.getLongArray("members")) members.add(BlockPos.of(packed));
        return new DecorationInstance(decorationId, BlockPos.of(tag.getLong("anchor")), List.copyOf(members));
    }
}
